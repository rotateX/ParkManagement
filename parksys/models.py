...
class ParkInfo(db.Model):
    ...
    create_by = db.Column(db.String(10), nullable=False, comment='创建人')
    update_on = db.Column(db.DateTime,  default=datetime.now, onupdate=datetime.now, comment='修改时间')
    update_by = db.Column(db.String(10), comment='修改人')
    delete_on = db.Column(db.DateTime, comment='删除时间')
    delete_by = db.Column(db.String(10), comment='删除人')
    state = db.Column(db.Integer, default=1, comment='状态(0删除，1启用)')
    remarks = db.Column(db.String(50), comment='备注')
    carinout = db.relationship('CarInOut', backref='park', lazy='dynamic')
    moncategory = db.relationship('MonthlyCategory', backref='park', lazy='dynamic')
...
class CarInOut(db.Model):
    ...
    park_id = db.Column(db.String(64), db.ForeignKey('park.id'))
    ...
    in_man = db.Column(db.String(10), comment='进场管理员')
    out_man = db.Column(db.String(10), comment='出场管理员')
    ...
    remarks = db.Column(db.String(50), comment='备注')
    parkingpay = db.relationship('ParkingPay', backref='car', lazy='dynamic')
...
class ParkingPay(db.Model):
    ...
    car_id = db.Column(db.String(64), db.ForeignKey('car.id'))
...
class MonthlyCategory(db.Model):
    ...
    create_on = db.Column(db.DateTime, default=datetime.now, comment='创建时间')
    create_by = db.Column(db.String(10), comment='创建人')
    update_on = db.Column(db.DateTime, default=datetime.now, onupdate=datetime.now, comment='修改时间')
    update_by = db.Column(db.String(10), comment='修改人')
    delete_on = db.Column(db.DateTime, comment='删除时间')
    delete_by = db.Column(db.DateTime, comment='删除人')
    state = db.Column(db.Integer, comment='状态（0删除，1启用）')
    monapply = db.relationship('Monthly_Apply', backref='category', lazy='dynamic')
...
class Monthly_Apply(db.Model):
    ...
    remarks = db.Column(db.String(50), comment='备注')
    monpay = db.relationship('MonthlyPay', backref='apply', lazy='dynamic')
...
class MonthlyPay(db.Model):
    ...
    apply_id = db.Column(db.String(64), db.ForeignKey('apply.id'))
    order_id = db.Column(db.String(64), comment='订单号')
    mchorder_id = db.Column(db.String(64), comment='商户订单号')
